...
public class ListView extends MenuView {
    ...
    private void saveReportProvisions(){
        ...
        keyboard.nextLine();        
        System.out.println("\nPlease enter of the file to save the report: ");
        
        reportName = keyboard.nextLine();

        try{
        pWriter = new PrintWriter(reportName);
        
        Date now = new Date();
        pWriter.println("Provisions - Report - Date of Creation: " + now.toString());
        
        ArrayList<ListItem> listOfProvisions = theGame.getProvisions();

        for (int i = 0; i < listOfProvisions.size(); i++) {
            ListItem currentProvision = listOfProvisions.get(i);
            pWriter.println("Provision: " + currentProvision.getName() + ", Worth: " + currentProvision.getNumber());
        }
        
        pWriter.close();
        
        System.out.println("\nReport with provisions saved!");        
        }
